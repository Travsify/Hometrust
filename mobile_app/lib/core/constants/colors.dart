import 'package:flutter/material.dart';

class AppColors {
  // Brand Palette: Nigerian Proptech Aesthetic
  static const Color primary = Color(0xFF0D5C3A); // Forest Green
  static const Color primaryDark = Color(0xFF083C25);
  static const Color primaryLight = Color(0xFF107A4E);
  static const Color accentGold = Color(0xFFC9A227); // Warm African Gold
  static const Color accentGoldLight = Color(0xFFE5B83B);

  // Neutrals
  static const Color background = Color(0xFFF8FAFC);
  static const Color surface = Color(0xFFFFFFFF);
  static const Color cardBorder = Color(0xFFE2E8F0);
  
  // Slate Text
  static const Color textPrimary = Color(0xFF0F172A);
  static const Color textSecondary = Color(0xFF475569);
  static const Color textMuted = Color(0xFF94A3B8);

  // Status Badges
  static const Color emeraldBg = Color(0xFFECFDF5);
  static const Color emeraldText = Color(0xFF059669);
  static const Color amberBg = Color(0xFFFFFBEB);
  static const Color amberText = Color(0xFFD97706);
  static const Color roseBg = Color(0xFFFEF2F2);
  static const Color roseText = Color(0xFFDC2626);
  static const Color blueBg = Color(0xFFEFF6FF);
  static const Color blueText = Color(0xFF2563EB);
}
