...
export class VerificationsService {
  static async create(params: CreateVerificationRequestParams) {
    ...
    const urgencyFee = params.urgency === 'EXPRESS' ? 45000 : 25000;
    const deliveryFee = params.deliveryFee || 0;
    const totalFeeAmount = urgencyFee + deliveryFee;
  ...
  /**
   * Admin: Dispatch physical hard copy documents with courier tracking
   */
  static async dispatchCourier(id: string, data: { courierPartner: string; waybillNumber: string }, adminUser: any) {
    ...
    await AuditService.log({
      adminId: adminUser.id,
      adminEmail: adminUser.email,
      action: 'HARD_COPY_VERIFICATION_DISPATCHED',
      entityType: 'VERIFICATION_REQUEST',
      entityId: id,
      details: {
        code: request.verificationCode,
    ...
    await NotificationsService.createAndDispatch({
      userId: request.userId,
      title: `Hard Copy Dispatched: ${request.propertyName}`,
      message: `Your sealed title verification report has been dispatched via ${data.courierPartner} (Waybill: ${data.waybillNumber}). Delivery PIN: ${request.deliveryOtp}`,
      type: 'VERIFICATION',
      actionDetails: [
  ...
  /**
   * Confirm physical delivery with 4-Digit OTP Handover PIN
   */
  static async confirmDelivery(id: string, data: { otp: string }) {
    ...
    await NotificationsService.createAndDispatch({
      userId: request.userId,
      title: `Documents Delivered: ${request.propertyName}`,
      message: `Your hard copy verification documents have been safely delivered and confirmed.`,
      type: 'VERIFICATION',
    });
